...
const ShipmentDetails = () => {
  ...
  if (isError) return <ErrorState message="We couldn't fetch your shipments." technicalError={error?.message} onRetry={refetch} />;
  ...
  const status = parseData.isCompleted ? 'COMPLETE' : parseData.isPaused ? 'PAUSE' : 'CURRENT';
  const isFinish = parseData.isCompleted;
  return (
    <View className='px-4 flex-1'>
      <CustomHeader title='Shipment #SH-10025' hasBack description={status} descriptionClassName='text-custom-primary-color' />
